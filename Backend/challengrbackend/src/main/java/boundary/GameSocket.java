...
public class GameSocket {
    ...
    private void handleMessage(String message, Session session, Long playerId) {
        try {
            ...
            } else if (message.contains("\"type\":\"pushup-result\"")) {
                handlePushupResult(message, playerId);
    ...
    private void handlePushupResult(String message, Long playerId) {
        ...
        if (map.containsKey(fromId) && map.containsKey(toId)) {
            ...
            if (fromReps > toReps) {
                String winnerName = battle.getFromPlayer().getName();
                System.out.println("Pushup Battle " + battleId + " beendet. From:" + fromReps + " To:" + toReps + ", winner=" + winnerName);
                computeAndBroadcastResult(battle, List.of(winnerName));
            } else if (toReps > fromReps) {
                String winnerName = battle.getToPlayer().getName();
                System.out.println("Pushup Battle " + battleId + " beendet. From:" + fromReps + " To:" + toReps + ", winner=" + winnerName);
                computeAndBroadcastResult(battle, List.of(winnerName));
            } else {
                System.out.println("Pushup Battle " + battleId + " beendet mit Gleichstand! (" + fromReps + ") Löst Konflikt aus.");
                // Fake ein uneindeutiges Voting, damit das bestehende Konfliktsystem (Konfliktpunkte usw.) greift
                computeAndBroadcastResult(battle, List.of(battle.getFromPlayer().getName(), battle.getToPlayer().getName()));
            }
