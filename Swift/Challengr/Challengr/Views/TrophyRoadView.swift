...
struct TrophyRoadView: View {
    @State private var playerPoints: Int = 200
    ...
    // MARK: - Etappen zwischen den Boxen
    private func etappenBlock(for rank: TrophyRank) -> some View {
        let isCurrentRank = rank.id == currentRank.id
        let steps = 8
        ...
        return ZStack(alignment: .center) {
            ...
            if isCurrentRank {
                Circle()
                    ...
                    .shadow(color: rank.color.opacity(0.8), radius: 12)
                    .overlay(
                        Circle().stroke(rank.color, lineWidth: 3)
                    )
                    .offset(y: CGFloat(steps / 2) * 36)
